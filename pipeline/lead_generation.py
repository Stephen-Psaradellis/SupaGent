...
class LeadGenerator:
    ...
    async def _run_enrichers(
        ...
        return enriched
    async def _run_apollo_workflow(
        self, leads: List[Lead], context: LeadPipelineContext
    ) -> List[Lead]:
        """Run Apollo contact workflow to create contacts and return leads for created contacts."""
        if not leads:
            return []
        
        try:
            # Import the Apollo connector
            from integrations.apollo import ApolloConnector
        except ImportError:
            logger.warning("Apollo connector not available - skipping workflow")
            return []
        
        # Check if Apollo is configured
        if not context.settings.APOLLOIO_API_KEY:
            logger.info("Apollo API key not configured - skipping workflow")
            return []
        
        # Initialize Apollo connector
        connector = ApolloConnector(api_key=context.settings.APOLLOIO_API_KEY)
        
        # Use the first lead to determine industry (they should all be the same)
        industry = leads[0].industry if leads[0].industry else "general business"
        
        # Execute the workflow
        logger.info(f"Starting Apollo contact workflow for {industry} with {len(leads)} discovered leads")
        result = await connector.execute_workflow(
            industry=industry,
            location="Chicago",  # Default to Chicago as specified
            limit=min(len(leads), 100)  # Don't create more than we discovered
        )
        
        if not result.get("success", False):
            message = result.get("message", "Unknown error")
            logger.warning(f"Apollo workflow failed: {message}")
            return []
        ...
        for contact_data in created_contacts:
            ...
            lead = Lead(
                ...
                domain="",  # Apollo contacts may not have domains
                location="Chicago",
                industry=industry,
                email=contact_data.get("email"),
                source="apollo_workflow",
                linkedin_url=contact_data.get("linkedin_url"),
            )
            
            # Add Apollo-specific metadata
            lead.metadata["apollo_contact"] = contact_data
            lead.metadata["apollo_workflow_stats"] = result.get("stats", {})
