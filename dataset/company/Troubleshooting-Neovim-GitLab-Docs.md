# Troubleshooting Neovim | GitLab Docs

Source: https://docs.gitlab.com/editor_extensions/neovim/neovim_troubleshooting/

Troubleshooting Neovim | GitLab Docs
Troubleshooting Neovim
When troubleshooting the GitLab plugin for Neovim, you should confirm if an issue still occurs
in isolation from other Neovim plugins and settings. First, run the Neovim
testing steps
,
then the
GitLab Duo Code Suggestions troubleshooting steps
.
...
E149: Sorry, no help for gitlab.txt
...
Set the
vim.lsp
log level in your current buffer:
:
lua
vim.lsp
.
...
append
(
'$HOME/.local/share/nvim/site/pack/gitlab/start/gitlab.vim'
)
vim.cmd
(
'runtime plugin/gitlab.lua'
)
-- gitlab.config options overrides:
local
minimal_user_options
=
{}
require
(
...
and capture relevant output.
Redact any references to sensitive information, such as tokens beginning with
glpat-
.
Remove sensitive information from any Vim registers or log files.
Error:
GCS:unavailable
This error happens when your local project has not set a remote in
.git/config
.
