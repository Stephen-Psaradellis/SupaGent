# Reference architecture: Up to 500 RPS or 25,000 users | GitLab Docs

Source: https://docs.gitlab.com/administration/reference_architectures/25k_users/
...
.
Before deploying this architecture it’s recommended to read through the
main documentation
first,
specifically the
Before you start
...
sections.
Target load
: API: 500 RPS, Web: 50 RPS, Git (Pull): 50 RPS, Git (Push): 10 RPS
High Availability
: Yes (
Praefect
needs a third-party PostgreSQL solution for HA)
...
Also, the sizing depends on selected Load Balancer and additional factors such as Network Bandwidth. Refer to
Load Balancers
for more information.
...
Review the existing
technical limitations and considerations before deploying Gitaly Cluster
...
Generate a password hash for the PostgreSQL replication username/password pair. This assumes you will use the default
username of
gitlab_replicator
(recommended). The command will request a password
...
PostgreSQL, with Patroni managing its failover, will default to use
pg_rewind
...
occurs, this is due to an existing
known issue
.
It will be resolved when you run a second
reconfigure
after the next step.
Create a
.pgpass
...
Ensure each node is talking to the current primary:
...
third party external service for the Redis Cache and Persistence instances
...
Edit
/etc/gitlab/gitlab.rb
and add the same contents as the primary node in the previous section replacing
redis_master_node
with
redis_replica_node
:
...
the main
GitLab
database as a convenience unless
...
Gitaly Cluster (Praefect) nodes are configured in Praefect with a
virtual storage
...
for more information.
Redis is primarily single threaded and doesn’t significantly benefit from an increase in CPU cores. For this size of architecture it’s strongly recommended having separate Cache and Persistent instances as specified to achieve optimum performance.
Can be optionally run on reputable third-party load balancing services (LB PaaS). See
Recommended cloud providers and services
for more information.
Should be run on reputable Cloud Provider or Self Managed solutions. See
Configure the object storage
...
Example config file
An example for the GitLab Helm Charts targeting the 500 RPS or 25,000 users reference architecture configuration
