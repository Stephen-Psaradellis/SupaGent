...
Configuration: GitLab Self-Managed administrators can modify the
...
In
URL
, enter the URL of the webhook endpoint.
...
Optional. In
Secret token
, enter a token to validate requests.
In the
Trigger
section, select the events to trigger the webhook.
Optional. To disable SSL verification, clear the
Enable SSL verification
checkbox.
Select
Add webhook
.
The secret token is sent with the webhook request in the
X-Gitlab-Token
HTTP header.
...
In
URL
, enter the full URL of the webhook.
To define masked portions, select
Add URL masking
.
In
Sensitive portion of URL
, enter the part of the URL you want to mask.
In
How it looks in the UI
, enter the value to display instead of the masked portion.
...
For example, if you’ve defined variables
path
and
value
...
custom_webhook_template
removed.
JSON serialization of interpolated field values
introduced
in GitLab 18.4
with a flag
named
...
The resulting request payload for a
push
event is:
{
"event"
...
Support for this feature is proposed in
issue 463332
.
...
For example:
*-stable
to match branches ending with
-stable
.
production/*
to match branches in the
production/
namespace.
...
Regular expression
.
Enter a regex pattern that follows the
RE2 syntax
.
...
The
Recent events
section displays all requests made to a webhook in the last two days.
The table includes:
HTTP status code:
Green for
200
...
View details
for the event.
To send the request again with the same data and the same
Idempotency-Key
header, select
Resend Request
.
...
>
Webhooks
to see all webhooks for this project.
To test a webhook directly from the list of configured webhooks:
Locate the webhook you want to test.
From the
Test
dropdown list, select the type of event to test.
To test a webhook while editing it:
Locate the webhook you want to test, and select
Edit
.
Make your changes to the webhook.
Select the
Test
dropdown list, then select the type of event to test.
...
4xx
range) only for misconfigured webhooks.
For unsupported events, return
400
or ignore the payload.
Avoid
500
...
History
Introduced
for group webhooks in GitLab 15.10.
Disabled on GitLab Self-Managed
for project webhooks in GitLab 15.10
with a flag
named
...
in GitLab 17.11.
Changed
to become permanently disabled after 40 consecutive failures in GitLab 17.11.
...
Temporarily disabled
if they fail four consecutive times
Disabled
if they fail 40 consecutive times
...
Failure occurs when:
The webhook receiver returns a response code in the
4xx
or
5xx
range.
...
These webhooks might display four failures in
Recent events
even though the UI might state they have 40 failures.
Re-enable disabled webhooks
To re-enable a disabled webhook, send a test request.
The webhook is re-enabled if the test request returns a response code in the
2xx
range.
Delivery headers
History
...
"02affd2d-2cba-4033-917d-ec22d5dc4b38"
X-Gitlab-Event
Webhook type name. Corresponds to event types in the format
"<EVENT> Hook"
.
"Push Hook"
X-Gitlab-Event-UUID
Unique ID for non-recursive webhooks. Recursive webhooks (triggered by earlier webhooks) share the same value.
"13792a34-cac6-4fda-95a8-c58e00a3954e"
Idempotency-Key
Unique ID consistent across webhook retries. Use to ensure idempotency in integrations.
...
![
A Markdown image with a relative URL.
](
/uploads/$sha/image.png
)
The rewritten image reference in the webhook body would be:
![
A Markdown image with an absolute URL.
](
https://gitlab.example.com/-/project/:id/uploads/<SHA>/image.png
)
This example assumes:
GitLab is installed at
gitlab.example.com
.
The project ID is at
123
.
...
Related topics
Webhook events and JSON payloads
Webhook limits
