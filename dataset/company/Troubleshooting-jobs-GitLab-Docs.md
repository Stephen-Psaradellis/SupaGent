...
Source: https://docs.gitlab.com/ci/jobs/job_troubleshooting/#jobs-or-pipelines-run-unexpectedly-when-using-changes
...
use a previous SHA to calculate the diff. This calculation is equivalent to
git diff HEAD~
...
in the variable definition, but can become invalid when expanded in
script:
,
changes:
,
...
When the
DOCKERFILES_DIR
variable is expanded in the
changes:
section, the full
path becomes
path/to/files//*
. The double slashes might cause unexpected behavior
...
get_sources
job section fails because of an HTTP/2 problem
...
fatal: ...
You can work around this problem by configuring Git and
libcurl
to
use HTTP/1.1
.
...
resource_group
gets stuck, a
GitLab administrator can try run the following commands from the
rails console
:
...
message
You can receive this message and have a disabled
Run
button when trying to run a manual job if:
The target environment is a
protected environment
and your account is not included in the
Allowed to deploy
list.
The setting to
