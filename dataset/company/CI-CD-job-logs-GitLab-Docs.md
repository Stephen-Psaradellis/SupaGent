...
Source: https://docs.gitlab.com/ci/jobs/job_logs/#custom-collapsible-sections
...
A job log displays the full execution history of a
CI/CD job
.
View job logs
To view job logs:
Select the project for which you want to view job logs.
On the left sidebar, select
CI/CD > Pipelines
.
Select the pipeline you want to inspect.
In the pipeline view, in the list of jobs, select a job to view the job logs page.
...
Introduced
in GitLab 16.7.
You can view the contents of a job log in full screen mode by clicking
Show full screen
.
...
History
Support for output of multi-line command bash shell output
Introduced
in GitLab 16.5 behind the
GitLab Runner feature flag
,
FF_SCRIPT_SECTIONS
.
...
You can create
collapsible sections in job logs
...
date +%s
: Command that produces the Unix timestamp (for example
1560896352
).
my_first_section
: The name given to the section. The name can only be composed
of letters, numbers, and the
_
,
.
, or
...
characters.
\r\e[0K
: Escape sequence that prevents the section markers from displaying in the
rendered (colored) job log. They are displayed when viewing the raw job log, accessed
in the upper-right corner of the job log by selecting
Show complete raw
(
doc-text
).
...
\e[0K
: ANSI escape code to clear the line from the cursor position to the end of the line.
(
\e[K
alone does not work; the
0
must be included).
Sample raw job log:
...
To remove the
echo
statements that create the section markers from the job output,
...
section_start
"my_first_section"
"Header of the 1st collapsible section"
echo
"this line should be hidden when collapsed"
section_end
"my_first_section"
...
echo -e "\e[0Ksection_end:`date +%s`:my_first_section\r\e[0K"
Delete job logs
When you delete a job log you also
erase the entire job
.
...
removed
in GitLab 17.2.
You can generate a timestamp in the
ISO 8601 format
...
To enable timestamps in job logs, add a
FF_TIMESTAMPS
CI/CD variable
to your pipeline and set it to
true
.
For example,
...
Here’s an example log output with
FF_TIMESTAMPS
enabled:
...
in GitLab 18.0 or later
After upgrading a GitLab Self-Managed instance to 18.0 or later, you might see
This job does not have a trace
errors. This could be caused by a failed upgrade migration
on an instance with both:
Object storage enabled
Incremental logging previously enabled with the removed feature flag
