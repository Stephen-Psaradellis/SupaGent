...
History
Support for state names that contain periods introduced in GitLab 15.7
with a flag
named
allow_dots_on_tf_state_names
. Disabled by default.
Support for state names that contain periods
generally available
in GitLab 16.0. Feature flag
allow_dots_on_tf_state_names
removed.
Support for GitLab-managed OpenTofu and Terraform states
introduced
in GitLab 18.3. Requires GitLab CLI (
glab
) 1.66 or later.
...
OpenTofu state files are encrypted with the Lockbox Ruby gem when they are at rest on disk and in object storage with a key derived from the
db_key_base
application setting.
To decrypt a state file, GitLab must be available
.
...
set up Terraform/OpenTofu state storage
.
You must turn on the
Infrastructure
menu for your project:
Go to
Settings
>
General
...
Neither OpenTofu nor GitLab encrypts the plan file by default. If your OpenTofu
plan.json
or
plan.cache
files include sensitive data like passwords, access tokens, or certificates, you should
encrypt the plan output or modify the project visibility settings. You should also
disable
public pipelines
and set the
artifact’s access flag to ‘developer’
(
access: 'developer'
).
...
.gitlab-ci.yml
file.
Push your project to GitLab. This action triggers a pipeline, which
runs the
gitlab-tofu init
,
...
Trigger the manual
deploy
job from the previous pipeline. This action runs the
gitlab-tofu apply
...
The
gitlab-tofu
CLI is a wrapper around the
tofu
CLI.
Customizing your OpenTofu environment variables
You can use
OpenTofu HTTP configuration variables
when you define your CI/CD jobs.
To customize your
init
and override the OpenTofu configuration,
use environment variables instead of the
init -backend-config=...
approach.
When you use
-backend-config
...
plan
command.
Usually passed forward to the
apply
command.
This configuration can lead to problems like
being unable to lock the state files in CI jobs
.
...
) command always writes the plan output to a file named
plan.cache
.
To change the filename, set the
TF_PLAN_CACHE
environment variable in your CI/CD pipeline configuration. For example, to
set the filename to
my-plan.tfplan
:
variables
...
:
"my-plan.tfplan"
Do not set the output filename by passing the
-out=<filename>
option. GitLab commands override this option.
...
Ensure the OpenTofu state has been
initialized for CI/CD
.
...
Terraform states
.
Next to the environment you want to use, select
Actions
(
ellipsis_v
) and select
...
To initialize a backend with
glab
, run the following command:
glab opentofu init <old_state_name>
Manually with OpenTofu CLI
...
/terraform/state/old-state-name"
tofu init
\
-backend-config
=
...
/terraform/state/<new-state-name>"
tofu init
\
-migrate-state
\
...
5
If the backend is initialized successfully,
you receive the following response. If you type
yes
...
OpenTofu data source
.
In your
main.tf
or other relevant file, declare these variables. Leave the values empty.
variable
...
description
=
"Gitlab remote state file address"
}
variable
"example_username"
{
...
description
=
"Gitlab username to query remote state"
}
variable
...
.
username
: The username to authenticate with the data source. If you are using
a
...
authentication, this value is the token value (the token must have the
API
scope).
...
.
An epic exists
to track improvements to this UI.
...
\
--url
"https://gitlab.example.com/api/v4/projects/<your_project_id>/terraform/state/<your_state_name>/versions/<version_serial_number>"
...
\
--url
"https://gitlab.example.com/api/v4/projects/<your_project_id>/terraform/state/<your_state_name>/lock"
# Unlock a state file
curl --request DELETE --header
"Private-Token: <your_access_token>"
\
...
Related topics
Troubleshooting GitLab-managed Terraform state
Sample project: Terraform deployment of AWS EC2 instance in a custom VPC
