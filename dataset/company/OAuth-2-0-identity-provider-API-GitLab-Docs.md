...
(GitLab 17.1 and later) Secure flow oriented toward devices without browser access. Requires a secondary device to complete the authorization flow.
...
For production, use HTTPS for your
redirect_uri
.
...
The
CODE_CHALLENGE
is a URL-safe base64-encoded string of the SHA256 hash of the
CODE_VERIFIER
:
...
when hashed
and encoded using the previous Ruby snippet produces a
CODE_CHALLENGE
string
of
...
The
root_namespace_id
is the root namespace ID associated with the project. This optional parameter
should be used when
SAML SSO
is configured for the associated group.
...
RETURNED_CODE
in the following example), you can request an
access_token
, with
...
"access_token"
:
"c97d1fe52119f38c7f67f0a14db68d60caa35ddc86fd12401718b649dcfa9c68"
,
"token_type"
:
"bearer"
...
"refresh_token"
:
"803c1fd487fec35562c205dac93e9d8e08f9d3652a24079d704df3039df1158f"
,
"created_at"
:
1628711391
...
parameters
=
'client_id=APP_ID&client_secret=APP_SECRET&refresh_token=REFRESH_TOKEN&grant_type=refresh_token&redirect_uri=REDIRECT_URI'
RestClient
.
post
'https://gitlab.example.com/oauth/token'
...
verification_uri
is returned to the user. For example:
{
"device_code"
:
...
:
"https://gitlab.example.com/oauth/device?user_code=0A44L90H"
,
"expires_in"
:
300
...
user_code
, the device client
begins polling the token endpoint with the associated
device_code
...
For more information on each potential response, see the relevant
RFC spec for device authorization grant
and the
RFC spec for authorization tokens
.
Example response:
{
"error"
...
At this point, the device authentication flow is complete. The returned
access_token
...
A sample application that implements the client side device flow can be found at:
https://gitlab.com/johnwparent/git-auth-over-https
.
...
enterprise users
with
password authentication disabled for their group
.
...
You can set the username to any string value. You should use
oauth2
:
...
To verify the details of a token, use the
token/info
...
"uid"
:
"1cb242f495280beb4291e64bee2a17f330902e499882fe8e1e2aa875519cab33"
},
"created_at"
:
1575890427
...
and
expires_in_seconds
are included in the response but are now deprecated. The
scopes
field is an alias for
scope
, and the
expires_in_seconds
field is an alias for
...
.
Allow users to get, list, and delete registry objects through the
