# Contribute to GitLab Runner development | GitLab Docs

Source: https://docs.gitlab.com/runner/development/
...
The following instructions setup your Go environment using
asdf
...
The recommended environment on which to install Go and Rancher Desktop for development is a local laptop or desktop. It is possible to use nested-virtualization to run Rancher Desktop in the cloud (which runs
k3s
...
wget https://storage.googleapis.com/golang/go1.24.6.linux-amd64.tar.gz
...
.tar.gz"
wget https://github.com/mikefarah/yq/releases/download/latest/
${
YQ_BINARY
}
.tar.gz
...
wget https://storage.googleapis.com/golang/go1.24.6.darwin-amd64.tar.gz
sudo tar -C /usr/local -xzf go*-*.tar.gz
export
PATH
=
...
wget https://storage.googleapis.com/golang/go1.24.6.darwin-amd64.pkg
open go*-*.pkg
export
PATH
=
...
.tar.gz
FreeBSD
pkg install go-1.24.4 gmake git mercurial
export
PATH
=
"
...
and create a helper Docker image from it.
Compile GitLab Runner for
linux/amd64
.
Build a DEB package for Runner. The official GitLab Runner images are based on Alpine and Ubuntu,
...
adds a new mechanism for autoscaling which will work with all environments.
...
Build the plugin
To run GitLab Runner with a plugin, generate an executable binary and place it on your system’s
PATH
.
To generate the binary, ensure
$GOPATH/bin
is on your
PATH
, then use
go install
.
Each plugin contains a path to
./cmd/<plugin-name>
. For example, from the
fleeting-plugin-aws
directory:
cd
cmd/fleeting-plugin-aws/
go install
...
=
"fleeting-plugin-aws"
# Fleeting plugin name as built above [1].
[
runners
.
autoscaler
...
credentials_file
=
"/Users/josephburnett/.aws/credentials"
.
# Credentials which can scale an Autoscaling Group (ASG) [2].
name
=
"jburnett-taskrunner-asg"
# ASG name.
project
=
"jburnett-ad8e5d54"
...
=
"ubuntu"
# ASG instance template username for login.
[[
runners
.
autoscaler
...
development
.
This means that the runner defaults to pulling a
helper image
with the
latest
tag.
...
"
$(
make print_test_ldflags
)
"
-run TestDockerCommandBuildCancel -v ./executors/docker/...
In GoLand
Currently, GoLand doesn’t support dynamic Go tool arguments, so you’ll need to run
make print_ldflags
first
and then paste it in the configuration.
To use the debugger, make sure to remove the last two flags (
-s -w
).
Local Docker images for runner and helper
You can build runner and helper as a local Docker image using these commands:
make runner-local-image
# build only gitlab-runner:local
make helper-local-image
# build only gitlab-runner-helper:local
...
can be created with these
make
targets. The targets support only Linux image creation.
...
the base images container registry
.
Examples:
# Make an ubuntu-based runner and helper
LOCAL_FLAVOR
=
ubuntu make runner-and-helper-local-image
# Specify a version and flavor
RUNNER_IMAGES_VERSION
=
0.0.1
LOCAL_FLAVOR
=
...
Tests
The Runner codebase makes a distinction between
unit
and
integration tests
...
_test.go
and contain the following build directive in the header:
...
.
Other resources
Reviewing GitLab Runner merge requests
Add support for new Windows Version
