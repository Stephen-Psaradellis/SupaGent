...
Source: https://docs.gitlab.com/ci/runners/hosted_runners/linux/
...
Offering
: GitLab.com
Hosted runners on Linux for GitLab.com run on Google Cloud Compute Engine. Each job gets a fully isolated, ephemeral virtual machine (VM). The default region is
us-east1
.
...
--mtu=1400
in the client side Docker configuration.
For more details, see
issue 473739
.
Container images
As runners on Linux are using the
docker+machine
executor
,
you can choose any container image by defining the
image
in your
.gitlab-ci.yml
file.
...
Runners with any of the
saas-linux-<size>-<architecture>
tags are configured to run in
privileged
mode
to support
Docker-in-Docker
