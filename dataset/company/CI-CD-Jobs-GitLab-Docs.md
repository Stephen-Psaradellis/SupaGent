...
Source: https://docs.gitlab.com/ci/jobs/#job-names
...
CI/CD jobs are the fundamental elements of a
GitLab CI/CD pipeline
.
Jobs are configured in the
.gitlab-ci.yml
...
Have a
job log
with the full execution log for the job.
Jobs are defined with
YAML keywords
that define all aspects
...
when
jobs run.
Group jobs together in collections called
stages
.
...
Save files as
artifacts
which can be used by other jobs.
Add a job to a pipeline
To add a job to a pipeline, add it into your
.gitlab-ci.yml
file. The job must:
...
Have either a
script
section defining commands to run,
or a
trigger
section to trigger a
downstream pipeline
to run.
For example:
my-ruby-job
...
, jobs are sorted by status severity
with failed jobs appearing first, and then alphabetically by name.
Selecting an individual job shows you its
job log
, and allows you to:
Cancel the job.
...
as an
experiment
on GitLab.com and GitLab Self-Managed in GitLab 17.3
with flags
named
populate_and_use_build_names_table
for the API and
fe_search_build_by_name
for the UI. Disabled by default.
Generally available
in GitLab 18.5. Feature flags
populate_and_use_build_names_table
and
fe_search_build_by_name
removed.
...
By default, the filter shows only build jobs. To view trigger jobs, clear the filter, then select
Kind
>
Trigger
.
The
Kind
filter is available only for project jobs. It is not available in the
Admin
area.
...
History
Introduced
job source in GitLab 17.9
with a flag
named
populate_and_use_build_source_table
. Enabled by default.
Generally available
on GitLab.com, GitLab Self-Managed, and GitLab Dedicated in GitLab 17.11.
...
duo_workflow
: Job initiated by GitLab Duo Agent Platform.
external
...
: Job initiated by a scheduled scan execution policy.
trigger
...
To create a group of jobs, in the
.gitlab-ci.yml
file,
...
In the following example, these three jobs are in a group named
build ruby
:
build ruby 1/3
:
...
-
echo "ruby3"
The pipeline graph displays a group named
build ruby
with three jobs.
...
When you retry a
trigger job
that triggers a downstream pipeline:
...
In the pipeline widget, next to the job you want to retry, select
Run again
(
retry
).
Select the
Pipelines
tab, next to the job you want to retry, select
Run again
(
retry
).
...
From the pipeline graph, next to the job you want to retry, select
Run again
(
retry
).
...
Pipelines
.
Go to a merge request and select the
Pipelines
tab.
For the pipeline with failed or canceled jobs, select
Retry all failed or canceled jobs
(
retry
).
Cancel jobs
...
The job is marked as
canceling
.
The currently-running command is allowed to complete. The rest of the commands in the job’s
before_script
or
script
are skipped.
If the job has an
after_script
section, it always starts and runs to completion.
The job is marked as
canceled
.
...
after_script
.
If you need to cancel a job immediately without waiting for the
after_script
, use
force cancel
.
...
, this field is on the top bar.
From your merge request, do one of the following:
In the pipeline widget, next to the job you want to cancel, select
Cancel
(
cancel
).
Select the
Pipelines
tab, next to the job you want to cancel, select
Cancel
(
cancel
).
...
From the pipeline graph, next to the job you want to cancel, select
Cancel
(
cancel
).
...
Pipelines
tab.
For the pipeline you want to cancel, select
...
Force cancel immediately moves a job from the
canceling
state to
canceled
.
When you force cancel a job, the
job token
is immediately revoked.
If the runner is still executing the job, it loses access to GitLab.
The runner aborts the job without waiting for
after_script
to complete.
Prerequisites:
...
.
Deployment jobs
Deployment jobs are CI/CD jobs that use
environments
.
A deployment job is any job that uses the
environment
keyword and the
start
environment
...
deployment safety
settings like
preventing outdated deployment jobs
and
