...
Source: https://docs.gitlab.com/ci/components/#use-component-context-in-components
...
for more
dynamic behavior.
CI/CD components are similar to the other kinds of
configuration added with the
include
keyword
,
but have several advantages:
Components can be listed in the
CI/CD Catalog
.
...
that have the functionality you need in the
CI/CD Catalog
.
For an introduction and hands-on examples, see
Efficient DevSecOps workflows with reusable CI/CD components
.
For common questions and additional support, see the
FAQ: GitLab CI/CD Catalog
blog post.
Component project
History
The maximum number of components per project
changed
from 10 to 30 in GitLab 16.9.
The maximum number of components per project
changed
from 30 to 100 in GitLab 18.5.
...
Optional. After the project is created, you can
add a project avatar
.
Components published to the
CI/CD catalog
use both the description and avatar when displaying the component project’s summary.
Add a YAML configuration file for each component, following the
...
You can
use the component
immediately, but you might want to consider
publishing the component to the
CI/CD catalog
.
Directory structure
...
A
README.md
Markdown file documenting the details of all the components in the repository.
A top level
templates/
directory that contains all the component configurations.
In this directory, you can:
Use single files ending in
.yml
for each component, like
templates/secret-detection.yml
.
Create subdirectories with a
template.yml
for each component,
like
...
Optionally, each component can also have its own
README.md
file that provides more detailed information, and can be linked from the top-level
README.md
file. This helps to provide a better overview of your component project and how to use it.
You should also:
...
Add a
LICENSE.md
file with a license of your choice that covers the usage of your component.
For example the
MIT
or
Apache 2.0
...
The
my-component
component’s configuration is defined in a single file.
The
my-other-component
component’s configuration contains multiple files in a directory.
Only the
template.yml
file can be used by other projects using the component.
Use a component
Prerequisites:
If you are a member of a parent group that contains the current group or project:
You must have the minimum role set by the visibility level of the project’s parent group. For example, you must have at least the Reporter role if a parent project is set to
Private
.
To add a component to a project’s CI/CD configuration, use the
include: component
keyword. The component reference is formatted as
...
secret-detection
is the component name that is defined as either a single file
templates/secret-detection.yml
or as a directory
templates/secret-detection/
containing a
template.yml
.
1.0.0
...
When a pipeline starts, any included component configuration
merges
...
Similarly, a component using
extends
for configuration with the same name as a job in your pipeline
could extend the wrong configuration. Make sure your pipeline and the component do not share
any configuration with the same name, unless you intend to
override
the component’s configuration.
To use GitLab.com components on a GitLab Self-Managed instance, you must
mirror the component project
.
...
A tag, for example:
1.0.0
. If a tag and commit SHA exist with the same name,
...
main
. If a branch and tag exist with the same name,
the tag takes precedence over the branch.
~latest
or a partial semantic version, which selects the latest version within the specified pattern
published in the CI/CD Catalog. Use
~latest
only if you want to use the absolute
latest version at all times, which could include breaking changes.
~latest
does not include pre-releases, for example
1.0.1-rc
...
Introduced
in GitLab 16.11
You can use partial semantic version numbers and the keyword
~latest
when referencing
...
This ensures that when you use formats like
1.2
or
~latest
, you only pull components that have been validated and published to the catalog, rather than potentially untested code from any repository.
...
version
~latest
to select the latest released version
For example, a component has versions:
1.0.0
,
1.1.0
,
...
a pre-release version, specify the full version, for example
1.0.1-rc
.
...
. Disabled by default.
Components can access metadata about themselves with a component context
CI/CD expression
.
...
Declare which component context fields the component needs in the
spec:component
header.
spec:component
supports
...
reference
fields.
Reference the context fields using the CI/CD expression
...
You can also use component context to
reference versioned resources
.
Write a component
...
versions such as
~latest
or a Git reference. Using a release or Git SHA guarantees that you are fetching the same revision
...
For example, if you need to build an image, consider using
Buildah
instead of Docker, so that you don’t
...
If the project contains multiple components, use a
table of contents
to help users quickly jump to a specific component’s details.
Add a
## Components
section with sub-sections like
### Component A
for each component.
...
Do not duplicate input documentation in the
README
. Inputs appear automatically on the component page.
Instead, link to the published component.
Add a
## Contribute
section if contributions are welcome.
If a component needs more instructions, add additional documentation in a Markdown file
in the component directory and link to it from the main
README.md
file. For example:
...
component
:
$CI_SERVER_FQDN/$CI_PROJECT_PATH/my-component@$CI_COMMIT_SHA
inputs
:
stage
:
...
examples for testing a component
.
...
$CI_SERVER_FQDN
instead of your instance’s Fully Qualified Domain Name (like
gitlab.com
).
When accessing the GitLab API in your component, use the
$CI_API_V4_URL
instead of the
full URL and path for your instance (like
...
Ensure that the component and its testing pipeline work also
...
script
:
bundle exec rspec dir1/
rspec-2
:
script
:
bundle exec rspec dir2/
Instead, you can:
...
A common keyword with problematic hard-coded values is
stage
. If a component job’s
stage is hardcoded, all pipelines using the component
must
either define
the exact same stage, or
override
the configuration.
...
For example, to create a component with
stage
...
component
:
$CI_SERVER_FQDN/myorg/ruby/test@1.0.0
inputs
:
stage
:
...
stage
keyword, you should avoid hard-coding job names
...
Use
inputs
to allow your component’s users to define a specific job name, or a prefix for the
...
description
:
"Alternatively, define the job's name"
job-stage
:
default
:
...
When using CI/CD variables in a component, evaluate if the
inputs
keyword
...
Inputs are explicitly defined in the component’s
spec
section, and have better validation than variables.
...
component
:
$CI_SERVER_FQDN/path/to/project/my-scanner@1.0.0
inputs
:
scanner-output
:
...
Ask users to store sensitive values as
masked or protected CI/CD variables in project settings
.
CI/CD Catalog
Tier
: Free, Premium, Ultimate
...
The
CI/CD Catalog
is a list of projects with published CI/CD components you can use
...
the CI/CD Catalog beta Product Tour
.
...
To use automation to enable this setting, you can use the
mutationcatalogresourcescreate
GraphQL endpoint.
Issue 463043
proposes to expose this in the REST API as well.
Publish a new release
CI/CD components can be
used
without being listed in the CI/CD catalog.
...
Have a
README.md
file in the root directory for the commit SHA of the tag being released.
Have at least one
CI/CD component in the
templates/
directory
...
release
keyword
in a CI/CD job to create the release,
not the
Releases API
.
...
You should configure the tag pipeline to
test the components
before
...
description
:
"Release $CI_COMMIT_TAG of components in $CI_PROJECT_PATH"
Create a
new tag
for the release,
...
of components to the Catalog,
you must use
semantic versioning
. Semantic versioning is the standard
...
To remove a component project from the catalog, turn off the
...
To publish the component project in the catalog again, you need to
publish a new release
.
Verified component creators
History
Introduced for GitLab.com
in GitLab 16.11
...
GitLab-maintained (
tanuki-verified
): GitLab.com components that are created and maintained by GitLab.
GitLab Partner (
partner-verified
): GitLab.com components that are independently created
...
creates an
internal request issue (GitLab team members only)
on behalf of the verified partner.
GitLab Partner-created components are provided
as-is
...
Verified creator (
check-sm
): Components created and maintained by
...
Open GraphiQL in the instance with your administrator account, for example at:
https://gitlab.example.com/-/graphql-explorer
.
Run this query, replacing
root-level-group
with the root namespace of the component to verify:
mutation
{
...
The
Verified creator
badge displays next to the component names in the CI/CD catalog.
To remove the badge from a component, repeat the query with
UNVERIFIED
for
verificationLevel
.
Convert a CI/CD template to a component
Any existing CI/CD template that you use in projects by using the
include:
syntax
can be converted to a CI/CD component:
Decide if you want the component to be grouped with other components as part of
an existing
component project
, or
create a new component project
.
Create a YAML file in the component project according to the
directory structure
.
...
writing a component
.
Improve the configuration, for example by enabling
...
Leverage the
.gitlab-ci.yml
in the components repository to
test changes to the component
.
Tag and
release the component
.
You can learn more by following a practical example for
migrating the Go CI/CD template to CI/CD component
.
...
.
Create a group
to host the component projects (recommended group:
components
).
Create a mirror of the component project
in the new group.
Write a
project description
...
Publish
a new release
in the self-hosted component project by
running a pipeline
for a tag (usually the latest tag).
...
Audit and review component source code
: Carefully examine the code to ensure it’s free of malicious content.
...
Use pinned versions
: Pin CI/CD components to a specific commit SHA (preferred)
or release version tag to ensure the integrity of the component used in a pipeline.
Only use release tags if you trust the component maintainer. Avoid using
latest
.
Store secrets securely
: Do not store secrets in CI/CD configuration files.
...
: Run component jobs in temporary,
isolated environments when possible. Be aware of
security risks
with self-managed runners.
Securely handle cache and artifacts
: Do not pass cache or artifacts from other jobs
...
Review CI/CD component changes
: Carefully review all changes to the CI/CD component configuration
before changing to use an updated commit SHA or release tag for the component.
Audit custom container images
: Carefully review any custom container images used by the CI/CD component
...
: Ensure all CI/CD component project maintainers
and owners have
2FA enabled
,
or enforce
2FA for all users in the group
.
...
Protect the default branch, and protect all release branches
...
:
Sign all commits
to the component project.
Discourage using
latest
: Avoid including examples in your
README.md
that use
@latest
.
Limit dependency on caches and artifacts from other jobs
: Only use cache and artifacts
from other jobs in CI/CD components if absolutely necessary
Update CI/CD component dependencies
: Check for and apply updates to dependencies regularly.
...
content not found
message
You might receive an error message similar to the following when using the
~latest
or a partial semantic
version qualifier to reference a component hosted by a
catalog project
:
...
behavior
was updated
in GitLab 16.10. It now refers to the latest semantic version of the catalog resource. To resolve this issue,
...
and could receive an error like
Build component error: Spec must be a valid json schema
.
This error can be caused by an empty
spec:inputs
section. If your configuration
does not use any inputs, you can make the
spec
section empty instead. For example:
spec
:
---
my-component
