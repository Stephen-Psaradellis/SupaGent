...
<<<<<<< HEAD
marks the beginning of the conflict block.
...
The version of the conflicted lines you don’t want to keep.
The three conflict markers: the beginning, the end, and the
=======
line between
the two versions.
...
Command line: Provides complete control over complex conflicts. For more information, see
...
>
Merge requests
and find the merge request.
Select
Overview
, and scroll to the merge request reports section.
Find the merge conflicts message, and select
Resolve conflicts
.
...
When you’ve resolved all the conflicts, enter a
Commit message
.
Select
...
In the header for that file, select
Edit inline
to open the editor. In this
example, the conflict block begins at line 1350 and ends at line 1356:
After you resolve the conflict, enter a
Commit message
.
Select
Commit to source branch
...
message, you can:
In a comment in the merge request, run the
/rebase
quick action
.
In the merge widget, select
Rebase source branch
.
Rebase with Git
.
To troubleshoot CI/CD pipeline issues, see
Debugging CI/CD pipelines
.
Rebase in the GitLab UI
To trigger a rebase from the GitLab UI, use the
/rebase
quick action
, or the
...
On the
Overview
tab, scroll to the merge request widget and select
Rebase source branch
.
In a comment, enter
/rebase
...
when rebased through the UI
.
Related topics
Rebase and resolve conflicts
Introduction to Git rebase and force-push
Git applications for visualizing the Git workflow
