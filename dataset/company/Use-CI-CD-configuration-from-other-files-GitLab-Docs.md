...
Source: https://docs.gitlab.com/ci/yaml/includes/#use-rules-with-include
...
To include a single configuration file, use
include
...
-
'my-config.yml'
If the file is a local file, the behavior is the same as
include:local
.
If the file is a remote file, it is the same as
include:remote
.
...
include
type, each array item defaults to
include:local
or
include:remote
, as needed:
...
before_script
.
Content of a custom configuration file named
...
.gitlab-ci.yml
file
override the values defined in the
autodevops-template.yml
...
, that nested
include
configuration is merged first (recursively).
...
include
is merged together, the main configuration
is merged with the included configuration.
This merge method is a
deep merge
, where hash maps are merged at any depth in the
...
in another configuration. For example, for
include
...
default
:
# Override the included default
before_script
:
echo "Unit test default override"
unit-test-job
:
script
:
...
before_script
:
echo "Smoke test default override"
smoke-test-job
:
script
:
...
With:
unit-tests
included first, the contents of the
.gitlab-ci.yml
file is:
include
:
...
smoke-test.sh
unit-tests
included last, the contents of the
.gitlab-ci.yml
file is:
include
:
...
:
smoke-test.sh
If no file overrides the included configuration, the order of the
include
entries
...
You cannot use variables defined in jobs, or in a global
variables
section which defines the default variables for all jobs. Includes are evaluated before jobs,
so these variables cannot be used with
include
.
...
with a flag
named
ci_support_include_rules_when_never
. Disabled by default.
Support for
when: never
and
...
:
exit 0
In this example, GitLab checks for the existence of
file.md
in the current project.
Review your configuration carefully if you use
include
with
rules:exists
in an include file
...
-
file.md
In this example, GitLab searches for the existence of
file.md
in
my-group/other-project
on commit ref
...
To change the search context you can use
rules:exists:paths
with
...
is included when
Dockerfile
has changed and the pipeline source is a merge request event. The jobs in
builds3.yml
must also be configured to run for
