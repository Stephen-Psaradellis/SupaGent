...
Source: https://docs.gitlab.com/ci/jobs/job_control/#specify-variables-when-running-manual-jobs
...
conditions like the value of variables or the pipeline type with
rules
.
When using job rules, learn how to
avoid duplicate pipelines
. To control pipeline creation, use
workflow:rules
.
Create a job that must be run manually
You can require that a job doesn’t run unless a user starts it. This is called a
manual job
.
...
is
false
, which is the default setting for jobs that have
when: manual
defined inside
rules
.
...
run the manual job
.
Merge requests in projects with
Pipelines must succeed
enabled can’t be merged with a blocked pipeline.
The pipeline shows a status of
blocked
.
When using manual jobs in triggered pipelines with a
trigger:strategy
,
...
Select the
name
of the manual job in the pipeline view, not
Run
(
play
).
...
If you add a variable that is already defined in the CI/CD settings or
.gitlab-ci.yml
file,
the
...
From the job details page, select
Retry
(
retry
).
...
with
when: manual
to require confirmation for manual jobs.
...
:
production
To stop the active timer of a delayed job, select
Unschedule
(
time-out
).
This job can no longer be scheduled to run automatically. You can, however, execute the job manually.
To start a delayed job manually, select
Unschedule
(
time-out
) to stop the delay timer and then select
Run
(
play
).
Soon GitLab Runner starts the job.
Delayed jobs that are
archived
do not run.
...
You can run a
trigger
job multiple times in parallel in a single pipeline,
...
parallel:matrix
job
You can fetch artifacts from a job created with
parallel:matrix
by using the
dependencies
...
For example, to fetch the artifacts from the job with a
RUBY_VERSION
of
2.7
and
...
and
mac:build: [vultr, data]
jobs finish, without waiting for
linux:build
to finish.
The
production
...
You can further define the order of each parallel matrix job using
needs:parallel:matrix
.
For example:
build_job
