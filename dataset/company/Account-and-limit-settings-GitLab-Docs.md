...
Source: https://docs.gitlab.com/administration/settings/account_and_limit_settings/#limit-the-lifetime-of-access-tokens
...
Account and limit
.
Increase or decrease by changing the value in
Maximum attachment size (MiB)
.
If you choose a size larger than the configured value for the web server,
you may receive errors. For more information, see the
troubleshooting section
.
For GitLab.com repository size limits, see
...
.
For GitLab.com push size limits, see
accounts and limit settings
.
When you
...
Use
Git LFS
to add large files to a repository.
...
Your team develops apps which require large files to be stored in
the application repository.
Although you have enabled
Git LFS
to your project, your storage has grown significantly.
Before you exceed available storage, you set up a limit of 10 GB
per repository.
On GitLab Self-Managed and GitLab Dedicated, only a GitLab administrator can set those limits. Setting the limit to
0
means
...
From the Project’s homepage, go to
Settings
>
General
.
Fill in the
Repository size limit (MiB)
field in the
...
From the Group’s homepage, go to
Settings
>
General
.
...
The
Repository
size includes both Git repository files and
Git LFS
objects.
Compare the current usage to your configured repository size limit to estimate
remaining capacity.
...
For GitLab Dedicated, submit a
support ticket
to request a restart of your instance.
If
Remember me
is enabled, users’ sessions can remain active for an indefinite period of time.
For details, see
...
History
Introduced
in GitLab 18.0 with a
flag
named
session_expire_from_init
. Enabled by default.
Generally available
in GitLab 18.3. Feature flag
session_expire_from_init
removed.
...
.
Select the
Expire session from creation date
checkbox.
...
Remember me
checkbox on sign-in. Their session remains active for an indefinite
...
number of minutes of inactivity set when you
customize your session duration
.
On the left sidebar, at the bottom, select
Admin
. If you’ve
...
Select or clear the
Remember me
checkbox to turn this setting on or off.
...
History
Introduced
in GitLab 13.9 with a
flag
named
two_factor_for_cli
. Disabled by default. This feature flag also affects
...
section.
Fill in the
Session duration for Git operations when 2FA is enabled (minutes)
field.
Select
Save changes
.
...
for GitLab Self-Managed. Disabled by default.
Generally available
...
For personal access tokens for service accounts, use the
...
.
Select the
Personal / Project / Group access token expiration
checkbox.
Select
Save changes
.
...
To control the maximum access token lifetime, use the
...
The default prefix for personal access tokens is
glpat-
...
In the
Instance token prefix
field, enter your custom prefix.
Select
Save changes
.
Limit the lifetime of access tokens
...
History
Changed
the maximum allowable lifetime limit to an increased value of 400 days in GitLab 17.6
with a flag
named
...
section.
Fill in the
Maximum allowable lifetime for access tokens (days)
field.
Select
Save changes
.
...
setting controls whether users can register applications to use GitLab
...
The
OAuth authorizations
setting controls whether users can use the OAuth resource owner password
...
.
Select or clear the
Allow user to use resource owner password credentials flow without OAuth client credentials
checkbox.
Select
Save changes
.
...
Offering
: GitLab Self-Managed, GitLab Dedicated
To maintain integrity of user details in
audit events
,
...
.
Clear the
Allow new users to create top-level groups
checkbox.
...
previously set profile visibility
is selected.
Set profiles of new users to private by default
...
.
Select the
Make new users’ profiles private by default
checkbox.
Select
Save changes
.
If
Allow users to make their profiles private
is disabled, this setting is also disabled.
...
When attaching a file to a comment or reply in GitLab, the
max attachment size
...
If you receive intermittent push errors in your
Rails exceptions log
, like this:
...
Housekeeping
tasks may be causing your repository size to grow.
