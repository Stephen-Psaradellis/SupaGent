...
\
--url
"https://gitlab.example.com/api/v4/projects?name=<example-name>&description=<example-description>"
...
The resulting cURL request for a project with ID
5
and a group ID of
17
is then:
...
vs
iid
Some API resources have two similarly-named fields. For example,
issues
,
merge requests
, and
...
For example, suppose a project with
id: 42
has an issue with
id: 46
and
...
.
Not all resources with the
iid
field are fetched by
iid
. For guidance
...
data structures.
Namespaced paths
If using namespaced API requests, make sure that the
...
\
--form
"override_params[some_other_param]=some_value"
\
--url
...
vs
false
In API responses, some boolean fields can have
null
values.
A
null
boolean has no default value and is neither
true
nor
false
.
GitLab treats
null
values in boolean fields the same as
false
.
In boolean arguments, you should only set
true
or
false
values (not
...
\
--url
"https://gitlab.example.com/api/v4/projects/gitlab-org%2Fold-path-project"
The response is:
...
...
endpoint was
deprecated
for offset-based pagination in GitLab 16.5 and is planned for removal in 17.0. This change is a breaking change. Use keyset-based pagination for this endpoint instead.
The
users
...
The following example lists 50
namespaces
per page:
curl --request GET
\
...
There is a
max offset allowed limit
for offset pagination. You can change the limit in GitLab Self-Managed instances.
Pagination
Link
header
Link
headers
are returned with each
response. They have
rel
set to
prev
...
(
per_page=3
), and requests the second page (
page=2
) of
comments
of the issue with ID
8
...
Status: 200 OK
...
The link to the next page contains an additional filter
id_after=42
that
excludes already-retrieved records.
As another example, the following request lists 50
groups
per page ordered
by
name
...
\
--url
"https://gitlab.example.com/api/v4/groups?pagination=keyset&per_page=50&order_by=name&sort=asc"
...
...
The link to the next page contains an additional filter
cursor=eyJuYW1lIjoiRmxpZ2h0anMiLCJpZCI6IjI2IiwiX2tkIjoibiJ9
that
excludes already-retrieved records.
The type of filter depends on the
order_by
option used, and you can have more than one additional filter.
The
Links
