...
Source: https://docs.gitlab.com/api/personal_access_tokens/
...
integer or string
No
If defined, returns tokens owned by the specified user. Non-administrators can only filter their own tokens.
Example request:
curl --request GET
\
--header
...
\
--url
"https://gitlab.example.com/api/v4/personal_access_tokens?user_id=3&created_before=2022-01-01"
Example response:
[
{
"id"
...
401: Unauthorized
if a non-administrator uses the
user_id
attribute to filter for other users.
...
integer or string
yes
ID of a personal access token or the keyword
self
.
curl --request GET
\
...
\
--url
"https://gitlab.example.com/api/v4/personal_access_tokens/<id>"
...
404: Not Found
if the user is an administrator but the token does not exist.
Self-inform
History
Introduced
in GitLab 15.5
...
YYYY-MM-DD
). If the token requires an expiration date, defaults to 1 week. If not required, defaults to the
...
\
--url
"https://gitlab.example.com/api/v4/personal_access_tokens/<personal_access_token_id>/rotate"
Example response:
{
"id"
:
...
Rotate a personal access token with the
api
or
self_rotate
scope
...
\
--url
"https://gitlab.example.com/api/v4/personal_access_tokens/self/rotate"
...
\
--url
"https://gitlab.example.com/api/v4/personal_access_tokens/<personal_access_token_id>"
If successful, returns
204: No Content
.
Other possible responses:
...
History
Introduced
in GitLab 15.0. Limited to tokens with
api
scope.
Introduced
in GitLab 15.4, any token can use this endpoint.
...
\
--url
"https://gitlab.example.com/api/v4/personal_access_tokens/self/associations"
Example response:
{
"groups"
:
...
"web_url"
:
"http://gitlab.example.com/groups/test"
,
"name"
:
"Test"
...
"web_url"
:
"http://gitlab.example.com/groups/test/test_private"
,
"name"
:
"Test Private"
...
"web_url"
:
"http://gitlab.example.com/test/test_project"
,
"namespace"
:
{
