...
Source: https://docs.gitlab.com/user/profile/service_accounts/
...
You authenticate as a service account with a
...
Do not receive notification emails without
...
You can also manage service accounts through the
...
Instance-wide service accounts
...
.
You can assign service accounts to groups and projects using the UI or the
members API
.
For more information about using the UI, see
...
Delete account and contributions
.
Enter the name of the service account.
Select
...
For instance service accounts, use the
users API
.
For group service accounts, use the
service accounts API
.
...
By default, this date can be a maximum of 365 days later than the current date. In GitLab 17.6 or later, you can
extend this limit to 400 days
.
Select the
desired scopes
.
