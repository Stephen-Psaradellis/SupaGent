...
Source: https://docs.gitlab.com/ci/environments/#deployment-tier-of-environments
...
Prerequisites:
In a private project, you must have at least the Reporter role. See
...
To view a list of all manual jobs in a deployment pipeline, select the
Run
(
play
) dropdown list.
...
History
Changed
to persist arbitrary URLs in GitLab 15.2
with a flag
named
...
review apps
.
An environment has one of three states, depending on whether its
stop job
has run:
available
: The environment exists. There might be a deployment.
stopping
: The
on stop job
has started. This state does not apply when there is no on stop job defined.
stopped
: Either the
on stop job
has run, or a user manually stopped the job.
...
deploy
stage.
In the job, define the environment
name
and
url
. If an
...
To create a dynamic environment, you use
CI/CD variables
that are
...
. That makes it difficult to reference the URL in
the
.gitlab-ci.yml
file.
...
For an overview, see
Set dynamic URLs after a job finished
.
...
# In script, get the environment URL.
-
...
:
deploy.env
# Report back dotenv file to rails.
environment
:
name
:
...
:
$DYNAMIC_ENVIRONMENT_URL
# and set the variable produced in script to `environment:url`
on_stop
:
stop_review
stop_review
...
History
Renaming an environment by using the API was
deprecated
in GitLab 15.9.
Renaming an environment with the API
removed
in GitLab 16.0.
...
By default, all
CI/CD variables
are available to all jobs in a pipeline.
...
can be available for. The default environment scope is the
*
wildcard, so any job
can access the variable.
You can use specific matching to select a particular environment. For example, set
the variable’s environment scope to
production
to only allow jobs with an
environment
of
production
...
or
include
. The variables might not be defined when
GitLab validates the pipeline configuration at pipeline creation.
Search environments
History
Introduced
in GitLab 15.5.
Searching environments within a folder
was introduced in GitLab 15.7 with
Feature flag
enable_environments_search_within_folder
. Enabled by default.
Generally available
in GitLab 17.4. Feature flag
enable_environments_search_within_folder
removed.
...
Environments
.
In the search bar, enter your search term.
The length of your
search term should be 3 or more characters
.
...
review/test-app
.
Also searching with the folder name prefixed like
review/test
matches
review/test-app
.
...
When using the
on_stop
action to stop an environment, the job runs if it’s not
archived
.
Stop an environment by using the UI
To trigger an
on_stop
action and manually stop an environment from the
...
Environments
.
Next to the environment you want to stop, select
Stop
.
...
This behavior persists even if no explicit
on_stop
CI/CD job is defined.
However,
issue 428625
proposes to change this behavior
so that production and staging environments stop only if an explicit
on_stop
CI/CD job is defined.
...
:
manual
When using this functionality together with merge trains, the
stop
job triggers only if
duplicate pipelines are avoided
.
...
Each commit on a merge request triggers a
review_app
...
The
environment:action
keyword can be used to reset the time
...
When a environment has been
scheduled to stop after a specified time period
,
...
in the upper-right corner, select the thumbtack (
thumbtack
).
To override an environment’s expiration in the
.gitlab-ci.yml
:
Open the project’s
.gitlab-ci.yml
...
Update the
auto_stop_in
setting of the corresponding deploy job to
auto_stop_in: never
.
The
auto_stop_in
...
.
Select the date to use for determining which environments to consider stale.
Select
Clean up
.
...
introduced
in GitLab 16.9. Disabled by default.
Feature flag
environment_stop_actions_include_all_finished_deployments
removed
in GitLab 17.0.
You can define a stop job for the environment with an
on_stop
action
in the environment’s deploy job.
...
All
on_stop
actions for an environment must belong to the same pipeline. To use multiple
on_stop
actions in
downstream pipelines
, you must configure the environment actions in
the parent pipeline. For more information, see
...
script
:
echo "Delete the resources in cloud a"
environment
:
name
:
...
script
:
echo "Delete the resources in cloud b"
environment
:
name
:
...
:
manual
Stop an environment without running the
on_stop
action
...
You must
stop
the environment before it can be deleted.
...
Stopped
tab.
Next to the environment you want to delete, select
Delete environment
.
On the confirmation dialog, select
Delete environment
...
feature.
If an environment is configured to stop after a certain time period, jobs with the
access
or
prepare
action will reset the scheduled stop time. The
environment:auto_stop_in
from the most recent successful deployment job to the environment is used when resetting the scheduled time.
For example, if the most recent deployment used
auto_stop_in: 1 week
and is later accessed by a job with
action: access
, the environment will be rescheduled to stop one week from the completion of the accessing job.
To access an environment without changing the scheduled stop time, use the
verify
action.
...
critical alert
is detected.
For GitLab to select the appropriate environment for the rollback, the alert should contain a
gitlab_environment_name
key with the name of the environment.
...
Terminal
:
On the page for a specific environment, on the right, select
Terminal
(
terminal
).
...
action: stop
doesn’t run
In some cases, environments do not stop despite an
on_stop
job being configured. This happens when the job
with the
action: stop
is not in a runnable state due to its
stages:
or
needs:
configuration.
...
The job with the
action: stop
might have a dependency on a job that has not yet completed.
To ensure the
action: stop
...
manual
Error: job
would create an environment with an invalid parameter
...
in your
.gitlab-ci.yml
, ensure the value is one of the
supported tiers:
production
,
staging
...
When you create a new merge request with a branch name
bug-fix!
,
the
review
job tries to create an environment with
review/bug-fix!
.
However, the
!
