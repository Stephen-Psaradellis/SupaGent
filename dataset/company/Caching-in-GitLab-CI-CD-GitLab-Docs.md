...
Source: https://docs.gitlab.com/ci/caching/#use-a-variable-to-control-a-jobs-cache-policy
...
cache:key:files
: Generate keys linked to the content of specific files.
cache:key:files_commits
: Generate keys linked to the latest commit of specific files.
...
cache
keyword. Otherwise it is disabled.
...
If
cache-$CI_COMMIT_REF_SLUG
is not found, the job looks for
cache-$CI_DEFAULT_BRANCH
as a fallback option.
If
cache-$CI_DEFAULT_BRANCH
is also not found, the job looks for
cache-default
...
, per-cache fallback keys are appended
...
Introduced
in GitLab 16.1.
To reduce duplication of jobs where the only difference is the pull policy, you can use a
CI/CD variable
.
For example:
conditional-policy
...
If you are compiling C/C++ projects, you can use
Ccache
to
...
Use
cache
to reuse the created cache between jobs, for example:
job
:
cache
:
...
# Override compiler path with ccache (this example is for Debian)
-
...
# Compiler mtime might change in the container, use checksums instead
script
:
-
...
-
time make
# Actually build your code while measuring time and cache efficiency.
-
ccache --show-stats || true
If you have multiple projects in a single repository you do not need a separate
CCACHE_BASEDIR
for each of them.
...
variables
:
# Change pip's cache directory to be inside the project directory because GitLab can only cache local items.
PIP_CACHE_DIR
:
...
If your project uses
cURL
to download dependencies or files,
...
-
.curl-cache/
In this example cURL downloads a file from a webserver and saves it to a local file in
.curl-cache/
.
The
--remote-time
flag saves the last modification time reported by the server,
and cURL compares it to the timestamp of the cached file with
--time-cond
. If the remote file has
...
global fallback cache key
.
As an example, assuming that
cache.key
is set to
$CI_COMMIT_REF_SLUG
, and that you have two branches
main
and
feature
...
When a pipeline is triggered by a tag (like
$CI_COMMIT_TAG
), the cache suffix (
-protected
or
...
Each time you clear the cache manually, the
internal cache name
is updated. The name uses the format
cache-<index>
...
Use the
[runners.docker]
section of the runners’
config.toml
to configure a single mount point on the host that
is mapped to
/cache
in each container, such as
volumes = ["/mnt/gitlab-runner/cache-for-all-concurrent-jobs:/cache"]
.
