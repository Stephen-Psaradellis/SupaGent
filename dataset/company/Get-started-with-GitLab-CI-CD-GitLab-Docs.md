...
Stages define the order of execution. Typical stages might be
build
,
test
, and
...
In your
.gitlab-ci.yml
file, you can specify a container image you want to use when running the job.
...
For example, the
inputs
context allows you to access information passed into the
...
You can hard-code CI/CD variables in your
.gitlab-ci.yml
file, set them in your project settings,
...
CI/CD variables
CI/CD expressions
CI/CD expressions use the
$[[ ]]
syntax and are validated when you create a pipeline.
...
(
$[[ inputs.INPUT_NAME ]]
): Access typed parameters passed into configuration files with
include:inputs
or when a new pipeline is run
Matrix context
(
$[[ matrix.IDENTIFIER ]]
): Access matrix values in job dependencies to create 1:1 mappings between matrix jobs
For more information, see:
CI expressions
