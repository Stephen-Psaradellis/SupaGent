...
GitLab Runner to support
docker
commands. This method requires
privileged
mode.
If you want to build Docker images without enabling
privileged
mode on the runner,
you can use a
Docker alternative
...
use the
shell
executor. In this configuration, the
gitlab-runner
user runs
...
The Docker image includes all of the
docker
tools and can run
...
.
You should always pin a specific version of the image, like
docker:24.0.5
.
...
docker:24.0.5-cli
image (if none is specified at the job level).
...
certificates in that directory. For more information, see
...
=
[
"/runner/services/docker"
]
# Temporary volume shared between build and service containers.
The Docker-in-Docker service creates a
docker.sock
. The Docker client connects to
docker.sock
...
DOCKER_HOST
:
"unix:///runner/services/docker/docker.sock"
services
:
-
docker:24.0.5-dind
...
that you are using.
Register GitLab Runner from command line. Use
docker
and
privileged
mode:
...
You might need to configure proxy settings to use the
docker push
command.
For more information, see
...
You can use the
Kubernetes executor
to run jobs in a Docker container.
...
.
Cache
: Each job runs in a new environment. Because every build gets its own instance of the Docker engine, concurrent jobs do not cause conflicts.
However, jobs can be slower because there’s no caching of layers. See
...
: Because the
docker:24.0.5-dind
container and the runner container do not share their
root file system, you can use the job’s working directory as a mount point for
child containers. For example, if you have files you want to share with a
child container, you could create a subdirectory under
...
To mount the Docker socket with the Kubernetes executor, add
"/var/run/docker.sock"
to the
Volumes in the
...
section
.
To specify a volume mount, update the
values.yml
file
by using
Helm chart
...
in a project, it removes the GitLab Runner
containers.
...
/src:/home/app/src test-image:latest run_app_tests
...
Code Quality scanning using CodeClimate
, you must match host and container paths for proper execution. For more details, see
...
For more information, see
Install Docker Community Edition (CE) on Windows Server
.
To use Docker commands in your Windows-based container CI/CD jobs, you can bind-mount
\\.\pipe\docker_engine
into the launched executor container. Docker is then available in the context of the image.
The
Docker pipe binding in Windows
is similar to
Docker socket binding in Linux
and have similar
Known issues
as
Known issues with Docker socket binding
...
You can use the
Docker executor
to run jobs in a Windows-based container.
To mount the Docker pipe with the Docker executor, add
...
.
You can use
Runner operating in a Linux environment but targeting Windows nodes
To mount the Docker pipe with the Kubernetes executor, add
"\\.\pipe\docker_engine"
to the
Volumes in the
[[runners.kubernetes.volumes.host_path]]
...
When you migrate from
dockerd
to
containerd
, the AWS EKS bootstrapping script
...
with this script:
Write-Output
"Rename the just installed Docker Engine Service from docker to dockerd"
Write-Output
"because the Start-EKSBootstrap.ps1 stops and disables the docker Service as part of migration from dockerd to containerd"
Stop-Service
-Name
docker
dockerd
...
Docker pipe binding has the same set of security and isolation issues as the
Known issues with Docker socket binding
.
...
If you are a GitLab Runner administrator, you can specify the
command
to configure the registry mirror
for the Docker daemon. The
dind
...
mounts the file for
every
container created by GitLab Runner.
The
dind
...
do not work, because a fresh Docker daemon is started with the service. You should
authenticate with registry
.
Make Docker-in-Docker builds faster with Docker layer caching
When using Docker-in-Docker, Docker downloads all layers of your image every time you create a build. You can
make your builds faster with Docker layer caching
.
Use the OverlayFS driver
The instance runners on GitLab.com use the
overlay2
driver by default.
...
copies the file system on every run. You can avoid this disk-intensive operation by using a different driver, for example
overlay2
.
Requirements
...
On Ubuntu systems, do this by adding the following line to
/etc/modules
:
overlay
Use the OverlayFS driver per project
You can enable the driver for each project individually by using the
DOCKER_DRIVER
CI/CD variable
in
.gitlab-ci.yml
...
BuildKit
: Includes rootless BuildKit options that eliminate Docker daemon dependency.
Buildah
: Build OCI-compliant images without requiring a Docker daemon.
Buildah example
To use Buildah with GitLab CI/CD, you need
a runner
with one
...
.
In the last step, Buildah uses the
Dockerfile
under the
...
tutorial for using Buildah to build images in rootless container
.
...
The following error might appear when you run a
docker
...
:
Get
"http://%2F%2F.%2Fpipe%2Fdocker_engine/v1.44/version"
:
open
//./
pipe
...
To solve the problem, use the workaround described in
Use the Kubernetes executor with Docker pipe binding
.
