...
Source: https://docs.gitlab.com/ci/testing/unit_test_reports/#add-screenshots-to-test-reports
...
To make a job fail when tests fail, your job’s
script
must exit with a non-zero status.
GitLab Runner uploads your test results in JUnit XML format as
artifacts
.
...
For test case limits, see
Maximum test cases per unit test report
.
...
testsuite
name
Test suite name (parsed but not displayed in UI)
testcase
classname
Test class or category name (used as the suite name)
testcase
name
...
system-out
Element content
System output and attachment tags (only parsed from
testcase
elements)
system-err
Element content
System error output (only parsed from
testcase
elements)
The
testcase classname
attribute is used as the suite name, not the
testsuite name
attribute.
...
Screenshot: Available in test details dialog (from
testcase system-out
)
Not displayed: “Authentication Tests” (from
testsuite name
)
Test result types
...
.
This count includes failed tests from completed pipelines, but not
blocked pipelines
.
Support for blocked pipelines is proposed in
issue 431265
.
...
to your test job.
Specify the path to your XML test report files.
Optional. To make report files browsable, include them with
artifacts:paths
.
Optional. To upload reports even when jobs fail, use
...
In the
Tests
tab of pipeline details after your test job completes.
In the
Test summary
panel of merge requests after your pipeline completes.
View test results in merge requests
View detailed information about test failures in merge requests.
The
Test summary
panel shows an overview of your test results,
including how many tests failed and passed.
To view test failure details:
In a merge request, go to the
Test summary
panel.
To expand the
Test summary
...
to go to the
Tests
tab in the pipeline details.
Copy failed test names
Copy test names to rerun them locally for debugging.
Prerequisites:
Your JUnit report must include
<file>
attributes for failed tests.
...
Select
View details
next to the test you want to copy.
In the dialog, select
Copy test name to rerun locally
(
copy-to-clipboard
).
...
Go to your pipeline details page.
Select the
Tests
tab.
Select any test suite to see individual test cases.
You can also retrieve test reports with the
Pipelines API
.
Test timing metrics
...
In your JUnit XML file, add attachment tags with screenshot paths relative to
$CI_PROJECT_DIR
:
<testcase
time=
...
In your
.gitlab-ci.yml
file, configure your job to upload screenshots as artifacts:
Specify the path to your screenshot files.
Optional. Use
artifacts:when: always
to upload screenshots when tests fail.
For example:
ruby
:
stage
...
when you select
View details
for a failed test in the
Test summary
panel.
Troubleshooting
Test report appears empty
You might see an empty
Test summary
panel in merge requests.
This issue occurs when:
Report artifacts have expired.
JUnit files exceed size limits.
To resolve this issue, set a longer
expire_in
value for the report artifact,
...
Support for custom limits is proposed in
epic 16374
.
...
Test summary
panel at all in merge requests.
