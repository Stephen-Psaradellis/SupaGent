# External authorization control | GitLab Docs

Source: https://docs.gitlab.com/administration/settings/external_authorization/
...
History
Moved
from GitLab Premium to GitLab Free in 11.10.
...
. Read more about the logs GitLab keeps in
the
...
Introduced
in GitLab 15.9.
Deploy tokens no longer being able to access container or package registries
introduced
in GitLab 16.0.
...
Select
Allow deploy tokens and deploy keys to be used with external authorization
.
Select
Save changes
.
...
The
user_ldap_dn
is optional and is only sent when the user is signed in
through LDAP.
identities
contains the details of all the identities associated with the
...
"reason"
:
"You are not allowed access to this project."
}
