# Reference architecture: Up to 1000 RPS or 50,000 users | GitLab Docs

Source: https://docs.gitlab.com/administration/reference_architectures/50k_users/
...
sections.
Target load
: API: 1000 RPS, Web: 100 RPS, Git (Pull): 100 RPS, Git (Push): 20 RPS
High Availability
: Yes (
Praefect
...
Edit
/etc/gitlab/gitlab.rb
and add the same contents as the primary node in the previous section by replacing
redis_master_node
with
redis_replica_node
:
...
Copy the
/etc/gitlab/gitlab-secrets.json
file from the first Linux package
...
Example config file
An example for the GitLab Helm Charts targeting the 1000 RPS or 50,000 users reference architecture configuration
