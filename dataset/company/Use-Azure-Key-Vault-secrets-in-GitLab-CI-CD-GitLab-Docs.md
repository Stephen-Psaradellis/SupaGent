# Use Azure Key Vault secrets in GitLab CI/CD | GitLab Docs

Source: https://docs.gitlab.com/ci/secrets/azure_key_vault/
...
History
Introduced
in GitLab and GitLab Runner 16.3. Due to
issue 424746
this feature did not work as expected.
Issue 424746
resolved and this feature made generally available in GitLab Runner 16.6.
...
Key Vault
on Azure.
Your IAM user must be
granted the
Key Vault Administrator
role assignment
for the
resource group
assigned to the Key Vault. Otherwise, you can’t create secrets inside the Key Vault.
...
Add
CI/CD variables to your project
to provide details about your Vault server:
AZURE_KEY_VAULT_SERVER_URL
: The URL of your Azure Key Vault server, such as
...
To use multiple secrets from Azure Key Vault in the same job, define each secret under the
secrets
keyword:
job
:
...
In these examples:
aud
is the audience, which must match the audience used when
creating the federated identity credentials
name
is the name of the secret in Azure Key Vault.
version
is the version of the secret in Azure Key Vault. The version is a generated
...
Troubleshooting
Refer to
OIDC for Azure troubleshooting
for general
...
known issue
in GitLab Runner where the JWT token isn’t parsed correctly.
...
ForbiddenByRbac
If your Azure Key Vault is using RBAC, you must add the
Key Vault Secrets User
role assignment to your Azure AD
application.
For example:
appId
=
$(
az ad app list --display-name gitlab-oidc --query
'[0].appId'
-otsv
)
