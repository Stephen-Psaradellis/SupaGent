# CI/CD expressions | GitLab Docs

Source: https://docs.gitlab.com/ci/yaml/expressions/
...
Configuration expressions use the
$[[ ]]
syntax and are evaluated at pipeline creation time (compile-time).
...
: Values are resolved when the pipeline configuration is created,
...
Static resolution
: Cannot perform dynamic logic or access runtime job state.
...
$[[ inputs.INPUT_NAME ]]
GitLab 17.0
Reference CI/CD inputs in reusable configurations.
Matrix context
$[[ matrix.IDENTIFIER ]]
GitLab 18.6 (Beta)
Reference
parallel:matrix
identifiers in job dependencies.
Component context
$[[ component.FIELD_NAME ]]
GitLab 18.6 (Beta)
Reference component metadata in component templates.
Inputs context
History
Introduced
...
input.
expressions have the following characteristics:
...
Function support: Predefined functions like
expand_vars
and
truncate
can manipulate values.
Scope: Available in the file where defined, or passed explicitly with
include:inputs
.
Matrix context
History
Introduced
in GitLab 18.6. This feature is in
beta
.
Use the
matrix.
...
values by using a
$[[ matrix.IDENTIFIER ]]
syntax. Use it in job dependencies to enable
dynamic 1:1 mappings between
parallel:matrix
jobs.
For example:
.os-arch-matrix
...
script
:
echo "Testing $OS on $ARCH"
parallel
:
!
reference [.os-arch-matrix, parallel]
test
:
script
:
...
Scoped to job-level
parallel:matrix
: Only values from the current job can be referenced.
...
To use component context, declare which fields are needed in the
spec:component
