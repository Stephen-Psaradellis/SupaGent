# Troubleshooting SSH | GitLab Docs

Source: https://docs.gitlab.com/user/ssh_troubleshooting/#general-ssh-troubleshooting
...
openssl rsa -in <your-key-file> -text -noout
|
grep
"Key:"
...
Ensure that your SSH key format is compatible with your server OS configuration. For example, ED25519 key pairs might not work on
some FIPS systems
.
...
. If you have several hosts ensure that permissions are correct in all hosts.
...
If GitLab does not have access to the host SSH keys, when you visit
gitlab.example/help/instance_configuration
, you see the following error message under the
SSH host key fingerprints
header instead of the instance SSH fingerprint:
...
webservice
section.
On GitLab Self-Managed instances, check the
/etc/ssh
directory for the host keys.
...
To run SSH in verbose mode, use the following command and replace
gitlab.example.com
with
