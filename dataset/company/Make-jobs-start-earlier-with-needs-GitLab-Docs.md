...
Source: https://docs.gitlab.com/ci/yaml/needs/
...
in a pipeline. Jobs run as soon as their dependencies are met, regardless of the pipeline’s
stages
...
as part of your main project. Using
needs
...
in a single pipeline, using only the
needs
...
test_d
deploy_d
You can improve job execution by using
needs
to relate the
a
jobs to each other
...
but
test_b
doesn’t need to wait, it can be configured to start as soon as
build_b
is finished,
