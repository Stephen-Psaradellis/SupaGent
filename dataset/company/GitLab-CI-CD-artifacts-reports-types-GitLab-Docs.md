# GitLab CI/CD artifacts reports types | GitLab Docs

Source: https://docs.gitlab.com/ci/yaml/artifacts_reports/
...
time for the artifacts, which overrides the instance’s
default setting
.
GitLab.com might have a
different default artifacts expiry value
.
Some
artifacts:reports
...
Combined reports in parent pipelines using
artifacts from child pipelines
is
not supported. Support for this feature is proposed in
epic 8205
.
...
report uses
pa11y
to report on the accessibility impact
...
as an artifact. This artifact is a JSON file output by the
Sitespeed plugin
.
GitLab can display the results of one report in the merge request
browser performance testing widget
.
GitLab cannot display the combined results of multiple
browser_performance
reports.
...
to collect
coverage report
in Cobertura or JaCoCo formats.
The
coverage_format:
Can be either
cobertura
...
The results of coverage reports appear in merge request
diff annotations
.
...
artifacts:access
to restrict the users that can download
the report artifacts in a job.
GitLab uses the
dotenv
gem
for handling dotenv files,
but applies additional restrictions beyond both
the original dotenv rules
and the gem’s implementation:
The maximum size of the dotenv file is 5 KB.
This limit
can be changed on GitLab Self-Managed
.
On GitLab.com,
...
application limit
.
Only UTF-8 encoding is
supported
.
The dotenv file cannot contain empty lines or comments (starting with
#
).
The variable name can contain only ASCII letters (
A-Za-z
), digits (
0-9
), and underscores (
...
Multiline values in the dotenv file
are not supported.
...
${var}
) in the dotenv file is not supported.
...
report collects
Metrics
. The collected Metrics report uploads to GitLab as an
artifact.
GitLab can display the results of one or more reports in the merge request
...
The
terraform
report obtains an OpenTofu
tfplan.json
file.
JQ processing required to remove credentials
.
