# Reference architecture: Up to 100 RPS or 5,000 users | GitLab Docs

Source: https://docs.gitlab.com/administration/reference_architectures/5k_users/
...
sections.
Target load
: API: 100 RPS, Web: 10 RPS, Git (Pull): 10 RPS, Git (Push): 2 RPS
High Availability
: Yes (
Praefect
...
.
Generate the password hash for the Praefect PostgreSQL username/password pair. This assumes you use the default
username of
praefect
(recommended). The command requests the password
<praefect_postgresql_password>
and confirmation. Use the value that is output by this command in the next
step as the value of
...
Back to set up components
Configure Sidekiq
Sidekiq requires connections to the
Redis
,
PostgreSQL
and
...
Example config file
An example for the GitLab Helm Charts targeting the 100 RPS or 5,000 users reference architecture configuration
