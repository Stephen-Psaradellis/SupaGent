# Reserved project and group names | GitLab Docs
...
where:
TOP_LEVEL_ROUTES
are names reserved as usernames or top-level groups.
PROJECT_WILDCARD_ROUTES
are names reserved for subgroups or projects.
GROUP_ROUTES
...
A_Garcia
a_garcia_1
Additionally, usernames and group names must contain only letters (
a-zA-Z
), digits (
0-9
), emoji, underscores (
...
Project names must contain only letters (
a-zA-Z
), digits (
0-9
), emoji, underscores (
