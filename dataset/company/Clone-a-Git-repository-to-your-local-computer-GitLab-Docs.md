...
Source: https://docs.gitlab.com/topics/git/clone/
...
Authenticate with GitLab by following the instructions in the
SSH documentation
.
On the left sidebar, select
Search or go to
and find the project you want to clone. If you’ve
turned on the new navigation
, this field is on the top bar.
On the project’s overview page, in the upper-right corner, select
Code
, then copy the URL for
Clone with SSH
.
...
cd
<new directory>
On Windows, if you enter your password incorrectly multiple times and an
Access denied
message appears,
...
Configure your browser for IDE protocols
.
...
From Visual Studio Code, with the
extension
installed, use the
extension’s
Git: Clone
...
All projects can be cloned into
IntelliJ IDEA
...
To ensure that the
Open in IDE
feature is working, you must configure your browsers to handle
custom application protocols, such as
vscode://
or
jetbrains://
.
...
Open link
to allow Firefox to open the application.
...
Search for or go to the
Applications
section.
Find and select your desired application in the list. For example,
vscode
or
jetbrains
.
Select Visual Studio Code or IntelliJ IDEA from the dropdown list, or select
Use other…
...
to open the application. Select
Open
to allow Chrome to open the application.
If you don’t want to be prompted again, select the checkbox to remember your choice.
Reduce clone size
...
blob:limit
=
1m git@gitlab.com:gitlab-com/www-gitlab-com.git
This would produce the following output:
Cloning into
'www-gitlab-com'
...
remote: Enumerating objects: 832467,
done
.
...
.
remote: Enumerating objects: 146,
done
.
remote: Counting objects: 100%
(
...
$ git clone --filter
=
blob:none --sparse git@gitlab.com:gitlab-com/www-gitlab-com.git
Cloning into
'www-gitlab-com'
...
remote: Enumerating objects: 678296,
done
.
remote: Counting objects: 100%
(
...
set
data --cone
remote: Enumerating objects: 301,
done
.
remote: Counting objects: 100%
(
...
filter spec. This mode of filtering uses a format similar to a
.gitignore
file to specify which files to include when cloning and fetching.
Partial clone using
sparse
filters is still experimental. It might be slow and significantly increase
Gitaly
resource utilization when cloning and fetching.
Filter all blobs and use sparse-checkout
instead, because
git-sparse-checkout
simplifies
...
rev-list-options
.
Create a filter spec. For example, consider a monolithic repository with many applications,
each in a different subdirectory in the root. Create a file
...
shared-component-a/
shared-component-b/
Clone and filter by path. Support for
--filter=sparse:oid
using the
...
git clone --sparse --filter
=
sparse:oid
=
master:shiny-app/.gitfilterspec <url>
# Optional: Check for missing objects that have not been fetched
git rev-list --all --quiet --objects --missing
=
print
|
wc -l
Git integrations with
bash
, Zsh, etc and editors that automatically
show Git status information often run
git fetch
which fetches the
...
remove filtering:
Fetch everything that has been excluded by the filters, to make sure that the
repository is complete. If
git sparse-checkout
was used, use
...
|
wc -l
Repack everything. This can be done using
git repack -a -d
, for example. This
should leave only three files in
.git/objects/pack/
:
A
pack-<SHA1>.pack
...
Delete the
.promisor
file. The previous step should have left only one
pack-<SHA1>.promisor
file, which should be empty and should be deleted.
Remove partial clone configuration. The partial clone-related configuration
