...
Source: https://docs.gitlab.com/ci/pipelines/pipeline_types/#branch-pipeline
...
Merge trains
These types of pipelines all appear on the
Pipelines
tab of a merge request.
Branch pipeline
Your pipeline can run every time you commit changes to a branch.
This type of pipeline is called a
branch pipeline
.
They display a
branch
...
and
protected runners
when the branch is a
protected branch
.
Tag pipeline
A pipeline can run every time you create or push a new
tag
.
This type of pipeline is called a
tag pipeline
...
A
merged results pipeline
runs on the result of the source and target branches merged together.
It’s a type of merge request pipeline.
These pipelines do not run by default. You must configure the jobs in the
.gitlab-ci.yml
file
...
might conflict with each other. Use
merge trains
to put merge requests in a queue.
