# GitLab Duo add-ons | GitLab Docs

Source: https://docs.gitlab.com/subscriptions/subscription-add-ons/
...
added to Core
in GitLab 18.3.
Added ability to disable seat assignment emails on self-managed
in GitLab 18.4.
...
To view which roles can access GitLab Duo Core, see
...
Usage limits, along with
the GitLab Terms of Service
,
...
From the
Payment method
dropdown list, select your payment method.
Select
Purchase seats
.
...
Purchase seats
.
In the Customers Portal, in the
Add additional seats
field, enter the number of seats. The amount
cannot be higher than the number of seats in the subscription associated with your group namespace.
In the
Billing information
section, select the payment method from the dropdown list.
Select the
Privacy Policy
and
Terms of Service
...
On the
GitLab Duo Pro
section of your subscription card select
Add seats
.
...
If the
GitLab Duo
menu item is not available, synchronize your subscription
after purchase:
On the left sidebar, select
Subscription
.
...
ensure GitLab Duo is set up for your GitLab Self-Managed instance
.
Assign and remove GitLab Duo seats in bulk
You can assign or remove seats in bulk for multiple users.
SAML Group Sync
GitLab.com groups can use SAML Group Sync to
manage GitLab Duo seat assignments
.
For GitLab.com
On the left sidebar, select
Search or go to
...
GitLab Duo
.
On the bottom right, you can adjust the page display to show
50
or
100
items to increase the number of users available for selection.
...
To assign seats, select
Assign seat
, then
Assign seats
to confirm.
To remove users from seats, select
Remove seat
, then
Remove seats
to confirm.
...
to confirm.
To the right of the user, turn on the toggle to assign a GitLab Duo seat.
Administrators of GitLab Self-Managed instances can also use a
Rake task
to assign or remove seats in bulk.
...
duo_add_on_groups
property
in your LDAP settings.
When
duo_add_on_groups
is configured, it becomes the single source of truth for Duo seat management among LDAP-enabled users.
...
History
Last GitLab Duo activity field
introduced
in GitLab 18.0.
Prerequisites:
You must purchase a GitLab Duo Pro or Enterprise add-on, or have an active GitLab Duo trial.
...
These fields use data from the
AddOnUser
type in the
GraphQL API
.
...
Assign seats
.
To filter by users assigned to a GitLab Duo seat, in the
Filter users
bar, select
Assigned seat
, then select
...
On the
Usage quotas
page, if you experience both of the following, you will be unable to use the UI to assign seats to your users:
The
Seats
tab does not load.
The following error message is displayed:
An error occurred while loading billable members list.
As a workaround, you can use the GraphQL queries in
this snippet
