...
RUN --mount=type=cache,cacheKey=pip-cache,target=/root/.cache/pip \
...
RUN --mount=type=cache,cacheKey=apt-cache2,target=/var/cache/apt \
    --mount=type=cache,cacheKey=apt-lists2,target=/var/lib/apt/lists \
    apt-get update && \
    apt-get install -y --no-install-recommends \
        apt-transport-https \
        ca-certificates \
...
RUN --mount=type=cache,cacheKey=playwright-cache,target=/root/.cache/ms-playwright \
    PLAYWRIGHT_BROWSERS_PATH=/root/.cache/ms-playwright \
